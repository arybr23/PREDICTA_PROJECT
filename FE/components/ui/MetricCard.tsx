...
const TONE_CONFIG: Record<MetricCardProps["tone"], { iconBg: string; iconColor: string; valueColor: string }> = {
  ...
  neutral: { iconBg: "bg-background",    iconColor: "text-text-secondary", valueColor: "text-text-primary" },
};
  ...
  return (
    <Card className="flex-1 min-w-0">
      <div className="flex items-center gap-2.5 mb-4">
        <span className={cn("inline-flex h-8 w-8 items-center justify-center rounded-lg shrink-0", config.iconBg)}>
      ...
      <p className={cn("text-2xl font-bold leading-tight mb-1 truncate", config.valueColor)}>{value}</p>
