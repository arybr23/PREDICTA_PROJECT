...
export default function StocksPage() {
  ...
  return (
    <DashboardLayout title="Stocks">
      <Card>
        <h3 className="text-card-title text-text-primary mb-5">
          Active Stock Balance Matrix
        </h3>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                <th className="pb-3 text-center text-caption font-medium uppercase tracking-wide text-text-secondary">
                  Current
                </th>
                <th className="pb-3 text-center text-caption font-medium uppercase tracking-wide text-text-secondary">
                  Predicted Need
            ...
            <tbody className="divide-y divide-border">
              {stockItems.map((item) => {
                ...
                return (
                  <tr key={item.id}>
                    <td className="py-3 text-body text-text-primary font-medium">
                      {item.name}
                    </td>
                    <td className="py-3 text-center text-body text-text-secondary">
      ...
      <Card>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-body text-text-primary font-medium">
                    Buy{" "}
                    <span className="text-primary font-bold">{purchase}</span>{" "}
                    {s.unit} {s.itemName}
                  </p>
                  <p className="text-caption text-text-secondary">
                    Need: {s.needed} {s.unit} | Current: {s.currentStock}{" "}
                    {s.unit}
                  </p>
                </div>
              </label>
        ...
        <div className="flex gap-3">
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-small-title text-white font-medium hover:bg-primary-dark transition-colors"
          >
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-xl border border-border px-5 py-2.5 text-small-title text-text-primary font-medium hover:bg-background transition-colors"
          >
